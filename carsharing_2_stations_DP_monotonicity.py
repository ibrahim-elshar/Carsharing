...
#                w2=np.array([i for i in itertools.product(range(w[1]+1), repeat=2) if sum(i)==w[1]])
                #
#                w1=np.array(list(sum_to_n(w[0], 2)))
#                w2=np.array(list(sum_to_n(w[1], 2)))
                #
                w1=np.array(range(w[0]+1))
                w2=np.array(range(w[1]+1))
#                vec=list(itertools.product(w1,w2))
                #
#                bn1 = ss.binom(w[0], self.prob_ij[0,0])
#                bn2 = ss.binom(w[1], self.prob_ij[1,0])
#                bn2 = ss.binom(w[1], self.prob_ij[1,1])
#                print("#################################")
#                temp=list(sum(i)[0] for i in itertools.product(w1,w2))
##                probwv=list((bn1.pmf(i[0][0]) * bn2.pmf(i[1][1])) for i in itertools.product(w1,w2))
#                prob=list((bn1.pmf(i[0][0]) * bn2.pmf(i[1][1]))*probEps for i in itertools.product(w1,w2))
#                newState =list(map(lambda x: x + state -w[0], temp))
##                print("newState="+str(newState))
#                reward = sum(w * bi_price)
##                print("reward="+str(reward))
##                prob=np.array(list(map(lambda x: x * probEps,probwv)))
##                print("prob="+str(prob))
##                print(np.array(list(map(lambda x: self.discount_rate * stateValue[t,x]+ reward, newState))))
#                returns+= sum(prob * np.array(list(map(lambda x: self.discount_rate * stateValue[t,x]+ reward, newState))))
##                print("returns="+str(returns))
#                print("#################################")
#                temp=np.array(list(sum(i)[0] for i in itertools.product(w1,w2)))
#                temp=np.array(list(sum(i) for i in vec))
                temp=w1[:, None] + w2
                temp = temp.ravel()
#                prob=np.array(list((bn1.pmf(i[0][0]) * bn2.pmf(i[1][1]))*probEps for i in itertools.product(w1,w2)))
#                prob=np.array(list((ss.binom.pmf(i[0],w[0], self.prob_ij[0,0]) * ss.binom.pmf(i[1],w[1], self.prob_ij[1,0]))*probEps for i in vec))
...
##                        print('probw={}'.format(probw))
#        #                print("temp="+str(temp))
#                        reward = sum(w * bi_price)
##                        print("reward="+str(reward))
#                        newState = (state +temp[0] - w[0]).astype(int)
##                        print("newState="+str(newState))
#                        returns +=probEps * probw *(reward + self.discount_rate * stateValue[t, newState])
##                        print("returns="+str(returns))
##        #                print("#################################")
        return returns
